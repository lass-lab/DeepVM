...
class TRAIN:
    ...
    class __SenderClass:
        ...
        def __method(self):
            for _ in range(self.__save_count):
                ...
                self.__MPI.send(data=data, dest=self.__destinaion_list[self.__MPI.rank], tag=0)
                del data
    ...
    def __init__(
        ...
        self.__shard_size = shard_size
        self.__destinaion_list = destination_list
        self.__Sender = self.__SenderClass(self.__save_count, self.__MPI, self.__destinaion_list)
    ...
    def save(self, obj):
        ...
        """
        assert self.__is_running, "You have not started the module yet. Please start the module first using init_DeepCheck() or start()."
...
def __init_train_node(
    ...
    train_node = TRAIN(communicator, save_count, int(os.environ["SHARD_RANK"]), shard_size, destination_list)
    return train_node
...
def init_DeepCheck(args, train_node_auto_start=True, **overrides) -> MPI:
    """
    ...
    if not train_node_auto_start:
        print("** DeepCheck: (Warning) \'train_node_auto-start\' is false. is set to false. Please make sure to call train_node.start() before training. **")
