...
class Trainer:
    ...
    def _run_batch (self, source, targets):
        ...
        loss.backward ()
        self.train_node.waiting_for_copying() # [DeepCheck] Wait until the copying process is complete.
        self.optimizer.step ()
        return loss.item()
...
def main (save_period: int, total_epochs: int, batch_size: int, snapshot_path: str, train_node: DeepCheck.TRAIN):
...
if __name__ == "__main__":
    ...
    communicator, train_node, _ = DeepCheck.init_DeepCheck(args) # [DeepCheck] Initialize the DeepCheck environment.

    main(args.save_period, args.total_epochs, args.batch_size, args.snapshot_path, train_node)

    DeepCheck.destroy_DeepCheck() # [DeepCheck] Destroy the DeepCheck environment.
